...
- Derive the sole destination-verifiable rational price from the exact midpoint of the frozen orders' feasible interval; if its reduced tuple is oversized, select normalized `1/1` when feasible and otherwise normalized `L`; treat `BatchClosed.referenceSqrtPriceX96` as telemetry only and reject every alternate feasible tuple.
...
- Derive the same frozen-feasible-interval midpoint or oversized bounded-fallback price that AuraHook recomputes, confirm the closure payout-encoding preflight and canonical vectors in `docs/agent.md`, run integer-identical pinned v4 swap math for only that candidate, apply full-width realized conservation, record the source block number and hash, and abandon publication if state changes before submission.
...
- Frozen-feasible-interval midpoint and oversized bounded-fallback parity, prospective empty-interval admission rejection, close-time pool-spot manipulation resistance, canonical deadline-horizon and individual-payout closure preflight, payout conservation, and rounding dust.
