...
- MVP canonical-price rule: derive the target from the exact rational midpoint of the frozen orders' feasible interval, never from a close-time pool spot value. Before any `BatchClosed`, every canonical individual payout must fit both the `uint128[]` schema and `type(int128).max`; an at-cap failure reverts the incoming order and a timeout-close failure becomes directly refundable.
